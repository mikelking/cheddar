...
class MSN_Kids_Listicle_Json_Feed extends MSN_Json_Base {
	...
	public function get_the_query() {
		$query = wp_cache_get( 'json_kids_listicle_feed_query', 'jsf' );
		...
		if ( $query == false ) {
			...
			wp_cache_set( 'json_kids_listicle_feed_query', $query, 'jsf',  static::CACHE_MAX_AGE );
		}
		if ( $this->debug_feed === true ) {
			echo '<PRE>';
			print_r( $query );
