...
class MDD_Reference_Handler {
	...
	public function get_post_fields( $post ) {
		...
		// Attempt to add the acf fields.
		$fields = $this->get_acf_post_fields( $fields, $post );
	 ...
	 *
	 * @param array    $sub_key The fields array used for the mdd references.
	 ...
	 * @returns array.
	 */
	public function add_acf_fields_recursive( $fields = array(), $field, $post_id, $parents = array(), $index = null ) {
	...
	public function get_attachment_url_regex() {
		...
		$this->attachment_url_regex = '!'
			...
			. 'url\(|' // ...or following a CSS 'url(' function,
			...
			. '(?:\1|[)?#]|$)' // Either a closing quote, a closing paren, the start of a query string or hash, or the end of $subject.
			. '!'; // End of regex.
		return $this->attachment_url_regex;
	}
	...
	public function detect_gutenberg_gallery_attributes( $refs, $subject ) {
		...
		preg_match_all( '|<!-- wp:gallery -->(?P<gallery>.*?)<!-- /wp:gallery -->|s', $subject, $matches );
