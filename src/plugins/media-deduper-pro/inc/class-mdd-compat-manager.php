<?php
/**
 * Media Deduper Pro: compatibility manager class.
 *
...
class MDD_Compat_Manager {
	...
	public function __construct() {
		$this->delete_mdd_pro_version_site_option();
		$this->check_index_version();
		add_action( 'admin_init', array( $this, 'prepare_admin_notices' ) );
	}

	/**
	 * Change 'mdd_pro_version' from a multisite 'site option' to a regular option.
	 *
	 ...
	 */
	public function delete_mdd_pro_version_site_option() {
		...
		if ( get_site_option( 'mdd_pro_version' ) ) {
			delete_site_option( 'mdd_pro_version' );
		}
	}

	/**
	 * If this plugin has been used on this site before, and the most recently active version used a
	 * different index format or indexed different data (e.g. the previous version didn't index
	 * Gutenberg blocks and the current one does), save an option that will prompt the user to
	...
	public function check_index_version() {
		...
		$last_active_version = get_option( 'mdd_pro_version' );
		...
		if ( ! $last_active_version ) {
			...
			if ( ! $has_meta ) {
				...
				update_option( 'mdd_pro_version', Media_Deduper_Pro::VERSION );
		...
		if ( version_compare( $index_version, $last_active_version ) > 0 ) {
			...
			Media_Deduper_Pro::delete_transients();
			...
			update_option( 'mdd_pro_reindex_updated', 1 );
	...
	public function prepare_admin_notices() {
		if ( get_option( 'mdd_pro_reindex_updated' ) ) {
			new MDD_Admin_Notice(
				sprintf(
					// translators: %s: Link URL.
					__( 'Thanks for updating Media Deduper Pro. Due to recent enhancements you’ll need to <a href="%s">regenerate the index</a>.', 'media-deduper' ),
